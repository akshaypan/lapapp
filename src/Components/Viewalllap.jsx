import React from 'react'
import NavigationBar from './NavigationBar'

const Viewalllap = () => {
  return (
    <div>

<NavigationBar/>
<div className="container">
      <div className="row">
        <div className="col col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
          <div className="row g-3">
           <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

           <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

          </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-3 col-xxl-3">

            <div class="card">
              <img src="https://images-cdn.ubuy.co.in/63534c8f63231e76f539154b-lenovo-business-s340-laptop-linux-mint.jpg" class="card-img-top" alt="..."/>
              <div class="card-body">
                <h5 class="card-title">Lenovo Business S340 Laptop</h5>
                <p class="card-text">Linux</p>
                <p class="card-text">Rs.50000</p>
                <a href="#" class="btn btn-primary">Buy now</a>
              </div>
            </div>

            </div>
            <div className="row g-3">
                <div className="col col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">

                <table class="table">
                        <thead>
                            <tr>
                            <th scope="col">SI No.</th>
                            <th scope="col">Laptop</th>
                            <th scope="col">Laptop company</th>
                            <th scope="col">Laptop price</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                            <th scope="row">1</th>
                            <td>Pro 14 Laptop</td>
                            <td>Redmi</td>
                            <td>10000</td>
                            </tr>
                            <tr>
                            <th scope="row">2</th>
                            <td>HP 15 Intel Core i3 7th Gen i3-7020U</td>
                            <td>HP</td>
                            <td>70000</td>
                            </tr>
                            <tr>
                            <th scope="row">3</th>
                            <td>Del latitude 3400</td>
                            <td>Ubuntu</td>
                            <td>645748</td>
                            </tr>
                        </tbody>
                    </table>

                </div>
            </div>
          </div>
        </div>
      </div>
     </div> 

    </div>
  )
}

export default Viewalllap